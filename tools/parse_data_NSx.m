function [outfile_path, parsed_chs] = parse_data_NSx(filename, max_memo_GB, output_name, channels)
...
% Determine if the files have already been parsed, by establishing the
% conventional path and checking.
...
outDirPath = dir(fullfile(parsedFolder, [file '_NSX_Ch*.NC5']));

if 0%~isempty(outDirPath)
  disp('Data already processed - returning appropriate paths')
  outfile_path = fullfile({outDirPath.folder}, {outDirPath.name})';
  return
end

% If the files don't exist, then proceed w/ parsing the input.
tcum=0
...
[outfile_path] = deal(cell(sum(NSx.MetaTags.ChannelID < 129),1));
parsed_chs = [];
for i = 1:nchan
...
metadata_file = [output_name '_TimeStamps.mat'];
metadata = struct;
